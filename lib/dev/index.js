...
App.prototype = {
  ...
  removeBrowser: function(browser){
    var index = this.runners.indexOf(browser);
    if (index > -1) {
      this.runners.splice(index, 1);
    }
    this.emit('runner-disconnected', browser);
  },
  connectBrowser: function(browserName, id, client){
    var existing = this.runners.some(function(runner){
    ...
    if (existing){
      ...
      var runner = this.runners.push(browser)
      this.emit('runner-connected', runner);
